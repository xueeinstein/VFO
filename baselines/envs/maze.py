# Modified from junhyukoh/value-prediction-network:maze.py
import os
import gym
import copy
import numpy as np
        ...
        for y in range(0, my):
            for x in range(0, mx):
                if maze_tensor[y + 1][x + 1][BLOCK] == 1:
                    if maze_tensor[y][x + 1][BLOCK] == 1 and \
                       maze_tensor[y + 2][x + 1][BLOCK] == 1 and \
                       maze_tensor[y + 1][x][BLOCK] == 0 and \
                       maze_tensor[y + 1][x + 2][BLOCK] == 0:
                        removable.append((y + 1, x + 1))
                    elif maze_tensor[y][x + 1][BLOCK] == 0 and \
                            maze_tensor[y + 2][x + 1][BLOCK] == 0 and \
                            maze_tensor[y + 1][x][BLOCK] == 1 and \
                            maze_tensor[y + 1][x + 2][BLOCK] == 1:
                        removable.append((y + 1, x + 1))
...
def visualize_maze(maze, img_size=320, goal2=None, agent_pos=None):
    my = maze.shape[0]
    mx = maze.shape[1]
    ...
    if goal2 is not None:
        y, x = goal2
        vis_maze[y][x] = colors[GOAL2]
    if agent_pos is not None:
        y, x = agent_pos
        vis_maze[y][x] = colors[AGENT]
    image = Image.fromarray(vis_maze)
    return image.resize((int(float(img_size) * mx / my), img_size),
                        Image.NEAREST)
...
class Maze(object):
    def __init__(self, size=10, num_goal=1, holes=0, task=0):
        self.size = size
        self.dx = [0, 1, 0, -1]
        self.dy = [-1, 0, 1, 0]
    ...
    def reset(self):
        ...
        if self.task != 0:
            # add single goal2
            goal2_idx = np.random.randint(self.num_goal)
            # [y, x]
            self.goal2 = self.obj_pos[GOAL][goal2_idx]
        else:
            self.goal2 = None
    ...
    def is_agent_on_end_of_corridor(self, direction):
        return self.is_end_of_corridor(self.agent_pos[0], self.agent_pos[1],
                                       direction)
    ...
    def is_object_reached(self, obj_idx):
        ...
        if self.task == 0:
            ...
            return self.maze[y][x][obj_idx] == 1 and \
                not self.agent_pos == self.goal2
        elif self.task == 2:
            return self.agent_pos == self.goal2
    ...
    def remaining_object(self, obj_idx):
        if self.task == 0:
            ...
            return len(self.obj_pos[obj_idx]) - 1
        elif self.task == 2:
            return 0
    ...
    def observation(self, clone=True):
        # Make maze env return image as observation, rather than the maze array
        # return np.array(self.maze, copy=clone)
        frame = self.visualize(img_size=84)
        frame = np.array(frame, np.float32)
        frame = np.reshape(frame, [84, 84, 3])
        return frame

    def visualize(self, img_size=320):
        return visualize_maze(self.maze, img_size=img_size, goal2=self.goal2,
...
class MazeEnv(object):
    def __init__(self, config="", verbose=0, task=0):
        self.config = BeautifulSoup(open(config), "lxml")
        # map
        ...
        self.spec = EnvSpec('standard_maze')
        # randomness
    ...
    def render(self):
        raise Exception("Don't support to render maze environment now.")
    ...
    def step(self, act):
        ...
        if self.maze.is_object_reached(GOAL):
            ...
            self.maze.remove_object(
                self.maze.agent_pos[0], self.maze.agent_pos[1], GOAL)
        ...
        if self.terminated:
            ...
            if self.log_t < self.log_freq:
                ...
                to_log["global/episode_reward"] = self.reward_mean(
                    self.log_freq)
                to_log["global/episode_length"] = self.length_mean(
                    self.log_freq)
                self.log_t = 0
        else:
    ...
    def tf_visualize(self, x):
        ...
        w = tf.minimum(tf.expand_dims(
            tf.reduce_sum(x, axis=xdim.ndims - 1), -1), 1.0)
...
class MixMaze(MazeEnv):
    def __init__(self, task=1, *args, **kwargs):
        """Same observation but have different tasks

        different task: seek and go to different color position
        task: 1 or 2"""
        super(MixMaze, self).__init__(*args, **kwargs)
        self.task = task
        self.spec = EnvSpec('mix_maze_task_{}'.format(task))
...
def make_maze(env_id, **kwargs):
    if 'config' in kwargs.keys():
        config = kwargs['config']
    else:
        config = 'deterministic'

    if env_id.startswith('stochastic_'):
        config = 'stochastic'
        env_id = env_id.strip('stochastic_')

    if config == 'deterministic' or config == 'stochastic':
        config = os.path.join(THIS_DIR, 'collect_{}.xml'.format(config))
    else:
        raise NotImplementedError

    kwargs.update({'config': config})

    if env_id == 'standard_maze':
        return MazeEnv(**kwargs)
    elif env_id == 'mix_maze_task_1':
        return MixMaze(task=1, **kwargs)
    elif env_id == 'mix_maze_task_2':
        return MixMaze(task=2, **kwargs)
    else:
        raise NotImplementedError


def make_maze_env(env_id, num_env, seed, start_index=0, **kwargs):
    """
    Create a wrapped, monitored SubprocVecEnv for Maze
    """
    ...
    def make_env(rank):
        def _thunk():
            ...
            env = Monitor(env, logger.get_dir() and path,
